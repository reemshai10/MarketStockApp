...
public class InputSellGoogle extends AppCompatActivity {
    public  int n = 0;
    public  int price = 0;
    DatabaseReference mRootRef = FirebaseDatabase.getInstance().getReference();
    DatabaseReference AmountRef = mRootRef.child("AmountGoogle");
    DatabaseReference GameMoneyRef = mRootRef.child("GameMoney");
    DatabaseReference GoogleRefOne = mRootRef.child("google");
    public String UpdateMoney ;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_input_sell_google);
        ...
        GoogleRefOne.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                String Number = snapshot.getValue(String.class);
        ...
        ButtonSend.setOnClickListener(v -> {
            ...
            if(n-num1<0){
                Toast.makeText(InputSellGoogle.this,"Cannot Sell More then " + n + " " + "Stocks!",Toast.LENGTH_LONG).show();
            }
            else {
                int sum = n - num1;
