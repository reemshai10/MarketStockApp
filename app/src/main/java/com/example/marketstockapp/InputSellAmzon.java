...
public class InputSellAmzon extends AppCompatActivity {
    public  int n = 0;
    public  int price =0;

    public String UpdateMoney ;
    DatabaseReference mRootRef = FirebaseDatabase.getInstance().getReference();
    DatabaseReference AmountRef = mRootRef.child("AmountAmazon");
    DatabaseReference GameMoneyRef = mRootRef.child("GameMoney");
    DatabaseReference AmazonRefOne = mRootRef.child("amzon");
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_input_sell_amzon);
        ...
        AmazonRefOne.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                String Number = snapshot.getValue(String.class);
        ...
        ButtonSend.setOnClickListener(v -> {
            ...
            if(n-num1<0){
                Toast.makeText(InputSellAmzon.this,"Cannot Sell More then " + n + " " + "Stocks!",Toast.LENGTH_LONG).show();
            }
            else {
                int sum = n - num1;
